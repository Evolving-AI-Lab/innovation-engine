...
struct Params
{
	...
	struct log
	{
		SFERES_CONST bool best_image = true;
	};
  ...
  struct pop
  {
    ...
    SFERES_CONST unsigned size = 100; //200;
...
const std::string Params::image::model_definition = "/project/EvolvingAI/anguyen8/model/imagenet_deploy_memory_data.prototxt";
...
int ParamsCaffe::image::category_id = -1;
...
int main(int argc, char **argv)
{
  ...
  // this file.
	typedef sferes::fit::FitDeepLearning<Params> fit_t;
...
//  typedef eval::Eval<Params> eval_t;
  typedef eval::MpiParallel<Params> eval_t;	// MPI
  ...
  typedef boost::fusion::vector<stat::BestFitImage<phen_t, Params>, stat::MeanFit<Params> >  stat_t;
  ...
  if (argc > 1) // if a number is provided on the command line
	{
  	int randomSeed = atoi(argv[1]);
		...
		// Use this as the class ID
		// For Rank_Simple evolving towards a single category
		Params::image::category_id = randomSeed; // Soccer Ball, refer to synset_words.txt
