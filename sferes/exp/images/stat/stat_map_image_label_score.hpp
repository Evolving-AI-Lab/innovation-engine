#ifndef STAT_MAP_IMAGE_LABEL_SCORE_HPP
#define STAT_MAP_IMAGE_LABEL_SCORE_HPP
...
namespace sferes
{
  namespace stat
  {
    SFERES_STAT(MapImageLabelScore, ImageStat)
    {
    public:
      ...
      MapImageLabelScore() : _xs(0), _ys(0) {}
      template<typename E>
			  ...
			  const int gen) const
			{
				...
				for (size_t i = 0; i < _xs; ++i)
				{
					for (size_t j = 0; j < _ys; ++j)
					{
						if (array[i][j])
						{
							int label = array[i][j]->fit().label();
							float score = array[i][j]->fit().score();
...
//							<< " " << array[i][j]->gen().get_nb_connections();
							...
							std::string category = boost::lexical_cast<std::string>(label);
