#ifndef STAT_MAP_PHYLOGENETIC_HPP
#define STAT_MAP_PHYLOGENETIC_HPP
...
namespace sferes
{
  namespace stat
  {
    SFERES_STAT(MapImagePhylogeneticStat, ImageStat)
    {
    public:
      ...
      MapImagePhylogeneticStat() : _xs(0), _ys(0) {}
      template<typename E>
				...
				for (size_t i = 0; i < _xs; ++i)
				{
					for (size_t j = 0; j < _ys; ++j)
					{
						if (array[i][j])
						{
							...
							// Record the top-1 label and score
							ofs
							<< " " << array[i][j]->fit().label()
							<< " " << array[i][j]->fit().score();
...
//							}
							...
							if (ea.gen() % Params::pop::dump_period == 0 || (ea.gen() <= 250 && (ea.gen() % 50 == 0)))
							{
