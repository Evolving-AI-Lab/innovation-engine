#ifndef STAT_MAP_IMAGE_L2_SCORE_HPP
#define STAT_MAP_IMAGE_L2_SCORE_HPP
...
namespace sferes
{
  namespace stat
  {
    SFERES_STAT(MapImageL2Score, ImageStat)
    {
    public:
			  ...
			  const int gen) const
			{
				...
				for (size_t i = 0; i < _xs; ++i)
				{
					for (size_t j = 0; j < _ys; ++j)
					{
						if (array[i][j])
						{
							float L1_norm = array[i][j]->fit().L1_norm();			// L1
							float L2_norm = array[i][j]->fit().L2_norm();			// L2
							float fitness = array[i][j]->fit().value(j);						// Fitness = Prob - L1 - L2
							float score = array[i][j]->fit().score(j);							// Softmax output

							ofs << j;		// This dimension is categorical (1-1000). No need to normalize to be [0, 1].
							ofs << " " << fitness;
							ofs << " " << L1_norm;
							ofs << " " << L2_norm;
							ofs << " " << score;
...
//							<< " " << array[i][j]->gen().get_nb_connections();
							...
							std::string image_fitness = boost::lexical_cast<std::string>(score);
